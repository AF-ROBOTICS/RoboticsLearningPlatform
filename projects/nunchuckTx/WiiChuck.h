 ...
 * 
 * Modified by Phillip Warner to use ints for the acceleration outputs
 *
 */
...
class WiiChuck {
    ...
    int readAccelX() {
       // total = 0; // accelArray[xyz][averageCounter] * FAST_WEIGHT;
        return angles[0] - ZEROX;
    }
    int readAccelY() {
        // total = 0; // accelArray[xyz][averageCounter] * FAST_WEIGHT;
        return angles[1] - ZEROY;
    }
    int readAccelZ() {
        // total = 0; // accelArray[xyz][averageCounter] * FAST_WEIGHT;
        return angles[2] - ZEROZ;
    }
   ...
   // }
    ...
    int readRoll() {
        return (int)(atan2(float(readAccelX()),float(readAccelZ()))/ M_PI * 180.0);
    }

    // returns pitch in degrees
    int readPitch() {        
        return (int) (acos(float(readAccelY())/RADIUS)/ M_PI * 180.0);  // optionally swap 'RADIUS' for 'R()'
    }
