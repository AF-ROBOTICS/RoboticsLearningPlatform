...
\SetHandoutTitle{Build BeyerBot}
...
\pagebreak
	...
	\begin{enumerate}
		...
		\item \textbf{Testing PCB:} Wire the motors, Arduino, battery, and switch to the PCB as shown in Figure. Download the \path{pcb\_test} folder from Canvas. Create the following folder:  \\\path{Program Files (x86)/Arduino/libraries/Motor} and move the \textit{Motor.h} file into it. Open the \textit{pcb\_test.ino} file. Compile and upload to the Arduino. If working correctly, the tires should go forward for 1 second, then left for 1 second, then right for 1 second, then backwards for 1 second. Disconnect the PCB from the Arduino.
