...
/* Updated to use distance values from the QTR-8RC Reflectance Sensor Array
*  from Pololu. Changes include changing to signed integer in cpp and header files 
*  used to be sum / avg 
*/

const int32_t Weight[8] = {332, 237, 142, 47, -47, -142, -237, -332};

int16_t QTRSensors::readLinePrivate(uint16_t * sensorValues, QTRReadMode mode,
                         bool invertReadings)
{
  bool onLine = false;
  int32_t sum = 0; // this is for the weighted total
  int16_t count = 0; // this is for the denominator, which is <= 64000
