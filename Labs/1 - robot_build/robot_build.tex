...
\pagebreak
	...
	\begin{enumerate}
		...
		\item \textbf{Testing PCB:} Wire the motors, Arduino, battery, and switch to the PCB as shown in Figure. Download the \path{robot_pcb} folder from \textbf{Teams} (\path{Labs/robot_pcb}). Create the following folder:  \\\path{C:Program Files (x86)/Arduino/libraries/Motor} and move the \textit{Motor.h} file into it. Open the \path{robot_pcb.ino} file. Compile and upload to the Arduino. If working correctly, the tires should go forward for 1 second, then left for 1 second, then right for 1 second, then backwards for 1 second. Disconnect the PCB from the Arduino.
		
		\begin{figure} [H]
			\centering
			\includegraphics[width=.75\textwidth]{6.jpg}
		\end{figure}
