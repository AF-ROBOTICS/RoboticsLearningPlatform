...
\SetHandoutTitle{BeyerBot Motors}
...
\begin{document}
	...
	\begin{itemize}
		...
		\item \textbf{Period/Frequency}: Time required to complete one full cycle (on/off). Frequency is the inverse of period and is constant. The Arduino pins used by the robot operate at 490 Hz.
	...
	\begin{figure} [H]
	\centering
	\includegraphics[height=3cm, keepaspectratio=false]{Figure1.PNG}
\end{figure}	
...
\end{figure}
	...
	\subsection{Code:}
	\begin{enumerate}
		\item Browse to Cadet Data and copy the Arduino sketch folder \textit{robot\_motors}: 
		\\ ``K:$\backslash$DF$\backslash$DFEC$\backslash$ECE210$\backslash$Labs$\backslash$Lab 8b - Robot Motors"
		\item Open the \textit{robot\_motors} sketch.
		\item In the \textit{loop()} function, update the five calls to \textit{robot.Motor\_Forward()} to drive the motor at the designated duty cycles. 
		\begin{itemize}
			\item The example provided, \textit{robot.Motor\_Forward(50, 50);}, will drive both the left and right motors at (50/255)\% of the duty cycle. This is equivalent to a duty cycle of about 20\%.
		\end{itemize}
	...
	\begin{enumerate}
		\item Disconnect \textbf{ONLY} the wire from \textit{Pin-3} on the Arduino.
		\item Insert male end of the Y-connector into \textit{Pin-3}.
		\item Connect the male end of the wire coming from your Printed Circuit Board (the one disconnected from \textit{Pin-3}) into the female end of the Y-connector.
		\item Connect the positive (red) end of the Moku:Lab probe to \textit{Pin-3} on the Arduino using the Y-connector.
...
\begin{enumerate}
	\item What is the frequency of the signal? \soln{.3in}{490 Hz}
